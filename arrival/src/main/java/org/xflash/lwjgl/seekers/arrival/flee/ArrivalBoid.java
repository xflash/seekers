...
public class ArrivalBoid {

    public static float SLOWING_RADIUS = 200f;
    ...
    public ArrivalBoid(float posX, float posY) {
        this(posX, posY, 20);
    }

    public ArrivalBoid(float posX, float posY, float totalMass) {
    ...
    private Vector2f seek(Vector2f target) {
        ...
        if (distance <= SLOWING_RADIUS) {
            desired.scale(MAX_VELOCITY * distance / SLOWING_RADIUS);
        } else {
    ...
    private Vector2f seek1(Vector2f target) {
        desired = target.copy().sub(position);
        desired.normalise();
        desired.scale(MAX_VELOCITY);
